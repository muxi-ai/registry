...
<div class="grid grid-cols-12 gap-x-8 my-5 relative">
    <div class="col-span-4 h-full">
        ...
        <div class="relative w-fit">
            ...
            <a href="https://github.com/<?php echo htmlspecialchars(tiny::user()->github_username); ?>" target="_blank" data-side="right" data-tooltip="github.com/<?php echo htmlspecialchars(tiny::user()->github_username); ?>" class="absolute bottom-0 -right-2">
        ...
        <?php
            if (tiny::user()->first_name || tiny::user()->last_name) {
                echo htmlspecialchars(tiny::user()->first_name); ?> <?php echo htmlspecialchars(tiny::user()->last_name);
            } else {
                echo htmlspecialchars(tiny::user()->registry_username);
            }
            ?>
            <?php if (tiny::user()->github_type === 'Organization'): ?>
        ...
        <div class="flex w-full items-center gap-1 mt-1.5">
            <p class="text-lg font-medium m-0! opacity-60 -mt-[3px]!">@<?php echo tiny::user()->registry_username; ?></p>
        ...
        <?php if (tiny::user()->bio): ?>
            <p class="my-5! text-base subpixel-antialiased leading-[1.8] opacity-80 text-pretty"><?php echo preg_replace('/@([a-zA-Z0-9_-]+)/', '<a href="/@$1" class="text-black dark:text-white font-medium hover:text-blue-500!">@$1</a>', htmlspecialchars(str_replace('@muxi-ai', '@muxi', tiny::user()->bio))); ?></p>
        <?php endif; ?>


        <ul class="mt-8! [&>li]:pl-px [&>li]:flex [&>li]:items-center [&>li]:space-x-2 [&>li>a]:flex [&>li>a]:items-center [&>li>a]:space-x-2 [&>li>svg]:size-4 [&>li>a>svg]:size-4 [&>li>strong]:font-semibold text-sm [&>li]:opacity-80 [&>li:hover]:opacity-100">
            <li>
            ...
            <li class="border-t mt-6! pt-6!">
                <a href="/@<?php echo htmlspecialchars(tiny::user()->github_username); ?>">
                    <svg class="p-px" fill="currentColor" viewBox="0 0 1000 1000" xmlns="http://www.w3.org/2000/svg"><path d="m1000 636.96c-1.1 52.91 4.54 105.18-19.82 152.24-24.74 45.13-74.7 75.98-108.33 59.12-69.87-36.04-2.78-213.39-137.7-142.79-29.28 17.14-55.51 43.55-73.86 72.74-37.34 58.84-33.72 126.47-73.02 181.07-20 27.42-53.64 41.04-87.28 41.13-33.64-.09-67.28-13.72-87.28-41.13-39.3-54.59-35.68-122.23-73.02-181.07-18.35-29.19-44.58-55.6-73.86-72.74-134.92-70.6-67.83 106.75-137.7 142.79-33.62 16.87-83.58-13.99-108.33-59.12-24.34-47.06-18.7-99.33-19.8-152.24v-363.18c6.03-68.29 68.2-111.07 126.38-144.79 45.49-26.32 131.22-75.8 131.22-75.8 77.56-47.35 156.03-53.93 242.4-53.65 86.37-.27 164.84 6.3 242.4 53.66 0 0 85.73 49.48 131.22 75.8 58.18 33.72 120.36 76.5 126.38 144.79v363.18z"/></svg>
                    <span>Public profile</span>
                </a>
            </li>
            <?php if (!empty(tiny::data()->adminOrgs)): ?>
            <?php if (count(tiny::data()->adminOrgs) == 1): ?>
                <li>
                    <a href="/@<?php echo htmlspecialchars(tiny::data()->adminOrgs[0]['registry_username']); ?>">
                        <svg viewBox="0 0 16 16" version="1.1" class="pl-px">
                            <path fill="currentColor" d="M1.75 16A1.75 1.75 0 0 1 0 14.25V1.75C0 .784.784 0 1.75 0h8.5C11.216 0 12 .784 12 1.75v12.5c0 .085-.006.168-.018.25h2.268a.25.25 0 0 0 .25-.25V8.285a.25.25 0 0 0-.111-.208l-1.055-.703a.749.749 0 1 1 .832-1.248l1.055.703c.487.325.779.871.779 1.456v5.965A1.75 1.75 0 0 1 14.25 16h-3.5a.766.766 0 0 1-.197-.026c-.099.017-.2.026-.303.026h-3a.75.75 0 0 1-.75-.75V14h-1v1.25a.75.75 0 0 1-.75.75Zm-.25-1.75c0 .138.112.25.25.25H4v-1.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 .75.75v1.25h2.25a.25.25 0 0 0 .25-.25V1.75a.25.25 0 0 0-.25-.25h-8.5a.25.25 0 0 0-.25.25ZM3.75 6h.5a.75.75 0 0 1 0 1.5h-.5a.75.75 0 0 1 0-1.5ZM3 3.75A.75.75 0 0 1 3.75 3h.5a.75.75 0 0 1 0 1.5h-.5A.75.75 0 0 1 3 3.75Zm4 3A.75.75 0 0 1 7.75 6h.5a.75.75 0 0 1 0 1.5h-.5A.75.75 0 0 1 7 6.75ZM7.75 3h.5a.75.75 0 0 1 0 1.5h-.5a.75.75 0 0 1 0-1.5ZM3 9.75A.75.75 0 0 1 3.75 9h.5a.75.75 0 0 1 0 1.5h-.5A.75.75 0 0 1 3 9.75ZM7.75 9h.5a.75.75 0 0 1 0 1.5h-.5a.75.75 0 0 1 0-1.5Z"></path>
                        </svg>
                        <span>@<?php echo htmlspecialchars(tiny::data()->adminOrgs[0]['registry_username']); ?> ›</span>
                    </a>
                </li>
            <?php else: ?>
            <li x-data="{ open: false }" class="block!" x-auto-animate>
                <a href="#" @click.prevent="open = !open">
                    <svg viewBox="0 0 16 16" version="1.1" class="pl-px">
                        <path fill="currentColor" d="M1.75 16A1.75 1.75 0 0 1 0 14.25V1.75C0 .784.784 0 1.75 0h8.5C11.216 0 12 .784 12 1.75v12.5c0 .085-.006.168-.018.25h2.268a.25.25 0 0 0 .25-.25V8.285a.25.25 0 0 0-.111-.208l-1.055-.703a.749.749 0 1 1 .832-1.248l1.055.703c.487.325.779.871.779 1.456v5.965A1.75 1.75 0 0 1 14.25 16h-3.5a.766.766 0 0 1-.197-.026c-.099.017-.2.026-.303.026h-3a.75.75 0 0 1-.75-.75V14h-1v1.25a.75.75 0 0 1-.75.75Zm-.25-1.75c0 .138.112.25.25.25H4v-1.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 .75.75v1.25h2.25a.25.25 0 0 0 .25-.25V1.75a.25.25 0 0 0-.25-.25h-8.5a.25.25 0 0 0-.25.25ZM3.75 6h.5a.75.75 0 0 1 0 1.5h-.5a.75.75 0 0 1 0-1.5ZM3 3.75A.75.75 0 0 1 3.75 3h.5a.75.75 0 0 1 0 1.5h-.5A.75.75 0 0 1 3 3.75Zm4 3A.75.75 0 0 1 7.75 6h.5a.75.75 0 0 1 0 1.5h-.5A.75.75 0 0 1 7 6.75ZM7.75 3h.5a.75.75 0 0 1 0 1.5h-.5a.75.75 0 0 1 0-1.5ZM3 9.75A.75.75 0 0 1 3.75 9h.5a.75.75 0 0 1 0 1.5h-.5A.75.75 0 0 1 3 9.75ZM7.75 9h.5a.75.75 0 0 1 0 1.5h-.5a.75.75 0 0 1 0-1.5Z"></path>
                    </svg>
                    <span>My Organizations <span class="absolute opacity-70 ml-1 top-[5px] badge rounded-full h-4 min-w-4 px-1 text-[8px] subpixel-antialiased"><?php echo count(tiny::data()->adminOrgs); ?></span></span>
                </a>
                <template x-if="open">
                    <ul class="my-0! ml-6 [&>li]:pl-px [&>li]:flex [&>li]:items-center [&>li]:space-x-2 [&>li>a]:flex [&>li>a]:items-center [&>li>a]:space-x-2 [&>li>svg]:size-4 [&>li>a>svg]:size-4 [&>li>strong]:font-semibold text-sm [&>li]:opacity-80 [&>li:hover]:opacity-100">
                    <?php foreach (tiny::data()->adminOrgs as $org): ?>
                    <li>
                        <a href="/@<?php echo htmlspecialchars($org['registry_username']); ?>">
                            <img src="<?php echo htmlspecialchars($org['avatar_url']); ?>" alt="<?php echo htmlspecialchars($org['login']); ?>" class="size-4 rounded-full" />
                            <span>@<?php echo htmlspecialchars($org['registry_username']); ?></span>
                        </a>
                    </li>
                    <?php endforeach; ?>
                    </ul>
            ...
            <?php endif; ?>
            <li>
                <a href="https://github.com/settings/connections/applications/<?php echo @$_SERVER['APP_GITHUB_APP_CLIENT_ID']; ?>" target="_blank">
            ...
            <li>
                <a href="<?php tiny::homeURL('/auth/logout'); ?>" class="text-red-500 font-medium">
    ...
    <div class="col-span-8 h-full lg:min-h-[64dvh]">
        <h2 class="text-2xl font-semibold font-sans! mb-2 px-0.5">My formations</h2>
            ...
            <div class="p-5 min-h-96 flex flex-col justify-center items-center text-center">
                ...
                <div class="max-w-sm mx-auto">
                    ...
                    <p class="mb-5 text-sm text-gray-500 dark:text-neutral-500">
                        To publish your first formation, go into the formation directory and type
